...
def is_float(string):
    """
    Return True if string can be converted to float, else return False.
...
def find_means_for_continuous_types(X):
    """
    Return the mean of every column.
...
def flatten_persons_inputs_for_model(person_inputs, means):
    """
    Preprocess of one person data. Rescale continuous features. Encode categoriacal features to one-hot.

    Params:
        person_inputs: 1D iterable
        means: list of the means of every column

    Return:
        float_inputs: list of floats
    """
    float_inputs = []
    for i in range(len(all_fields_shape)):
        ...
        if is_feature_continuous:
            ...
            if is_float(person_inputs[i]):
                scale_factor = 1 / (
                    2 * mean
                )  # we prefer inputs mainly scaled from -1 to 1.
...
def flatten_person_inputs_was_setup(person_inputs, distributions, person_idx):
    """
    Preprocess of one person data. Encode continuous and categoriacal features to one-hot.

    Params:
        person_inputs: 1D iterable
        distributions: 2D iterable
...
def convert_continuous_to_categorical(distributions):
    ...
    for i in range(len(all_fields_shape)):
        ...
        if is_feature_continuous:
            percentiles = (
                stats.rankdata(feature_dist, "min") - 1
...
def prepare_data(raw_data, means, wass_setup=False, filter_entries=True):
    """
    Create datasets for the protected feature, the rest of the features and the labels.

    Params:
        raw_data: 2D iterable
        means: list of floats
        wass_setup: boolean
    ...
    for person in range(len(X)):
        ...
        if (
            wass_setup and filter_entries and race not in ["White", "Black"]
        ):  # Wass only uses white or black
            bad_idxs.add(person)
            continue
        if (
            wass_setup
        ):  # Wass requires both different protected data (race + sex) and different formatting of inputs.
            formatted_X = flatten_person_inputs_was_setup(
                person_data, percentiles, person
            )
        else:
...
def get_adult_data(train_path, test_path, wass_setup=False):
    """
    Load adult dataset and return training and test splits.
    ...
    X_train, p_train, y_train = prepare_data(
        training_data, means, wass_setup=wass_setup
    )
    X_test, p_test, y_test = prepare_data(
        test_data, means, wass_setup=wass_setup, filter_entries=False
    )
